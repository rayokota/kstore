...
public class TestGet extends AbstractTest {
    ...
    public void testNoQualifier() throws IOException {
        ...
        for (int i = 0; i < numValues; ++i) {
            ...
            Assert.assertArrayEquals(
                values[i],
                CellUtil.cloneValue(
                    result.getColumnLatestCell(COLUMN_FAMILY, quals[i])));
        }
    ...
    public void testMaxResultsPerColumnFamily() throws IOException {
        ...
        for (int i = 0; i < maxColumns; ++i) {
            QualifierValue keyValue = keyValues.get(i);
    ...
    public void testEmptyValues() throws IOException {
        ...
        for (int i = 0; i < numValues; ++i) {
            Assert.assertTrue(result.containsColumn(COLUMN_FAMILY, quals[i]));
            Assert.assertArrayEquals(
                new byte[0], result.getValue(COLUMN_FAMILY, quals[i]));
        }
    ...
    public void testReadSpecialCharactersInColumnQualifiers() throws IOException {
        ...
        for (int i = 0; i < qualifiers.length; i++) {
            byte[] value =
                CellUtil.cloneValue(
                    result.getColumnLatestCell(COLUMN_FAMILY, qualifiers[i]));
