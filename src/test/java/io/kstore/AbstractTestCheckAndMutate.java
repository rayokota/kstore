...
public abstract class AbstractTestCheckAndMutate extends AbstractTest {
    ...
    public void testCheckAndPutSameQual() throws Exception {
        ...
        try (Table table = getDefaultTable()) {
            ...
            boolean success = checkAndPut(rowKey, COLUMN_FAMILY, qual, value2, put);
            Assert.assertFalse("Column doesn't exist.  Should fail.", success);
            success = checkAndPut(rowKey, COLUMN_FAMILY, qual, null, put);
            ...
            success = checkAndPut(rowKey, COLUMN_FAMILY, qual, value2, put);
            Assert.assertFalse("Wrong value should fail", success);
            success = checkAndPut(rowKey, COLUMN_FAMILY, qual, value1, put);
            Assert.assertTrue(success);
    ...
    public void testCheckAndDeleteSameQual() throws Exception {
        ...
        boolean success = checkAndDelete(rowKey, COLUMN_FAMILY, qual, value1, delete);
        Assert.assertFalse("Column doesn't exist.  Should fail.", success);
        success = checkAndDelete(rowKey, COLUMN_FAMILY, qual, null, delete);

        // Add a value and check again
        Put put = new Put(rowKey).addColumn(COLUMN_FAMILY, qual, value1);
        table.put(put);
        success = checkAndDelete(rowKey, COLUMN_FAMILY, qual, value2, delete);
        Assert.assertFalse("Wrong value.  Should fail.", success);
        success = checkAndDelete(rowKey, COLUMN_FAMILY, qual, value1, delete);
        Assert.assertTrue(success);
    ...
    public void testCheckAndPutDiffQual() throws Exception {
        ...
        boolean success = checkAndPut(rowKey, COLUMN_FAMILY, qual2, value2, put);
        Assert.assertFalse("Column doesn't exist.  Should fail.", success);
        success = checkAndPut(rowKey, COLUMN_FAMILY, qual2, null, put);
        Assert.assertTrue(success);

        // Fail on null check, now there's a value there
        put = new Put(rowKey).addColumn(COLUMN_FAMILY, qual2, value2);
        success = checkAndPut(rowKey, COLUMN_FAMILY, qual1, null, put);
        Assert.assertFalse("Null check should fail", success);
        success = checkAndPut(rowKey, COLUMN_FAMILY, qual1, value2, put);
        Assert.assertFalse("Wrong value should fail", success);
        success = checkAndPut(rowKey, COLUMN_FAMILY, qual1, value1, put);
        Assert.assertTrue(success);
    ...
    public void testCheckAndDeleteDiffQual() throws Exception {
        ...
        boolean success =
            checkAndDelete(rowKey, COLUMN_FAMILY, qual2, value2, delete);
        Assert.assertFalse("Column doesn't exist.  Should fail.", success);
        success = checkAndDelete(rowKey, COLUMN_FAMILY, qual2, null, delete);
        Assert.assertTrue(success);
        ...
        success = checkAndDelete(rowKey, COLUMN_FAMILY, qual2, null, delete);
        Assert.assertFalse("Null check should fail", success);
        success = checkAndDelete(rowKey, COLUMN_FAMILY, qual2, value1, delete);
        Assert.assertFalse("Wrong value should fail", success);
        success = checkAndDelete(rowKey, COLUMN_FAMILY, qual2, value2, delete);
        Assert.assertTrue(success);
        delete = new Delete(rowKey).addColumns(COLUMN_FAMILY, qual2);
        success = checkAndDelete(rowKey, COLUMN_FAMILY, qual1, null, delete);
        Assert.assertTrue(success);
        Assert.assertFalse("Row should be gone", table.exists(new Get(rowKey)));
    ...
    public void testCheckAndPutDiffRow() throws Exception {
        ...
        try {
            checkAndPut(rowKey2, COLUMN_FAMILY, qual, null, put);
        } catch (IOException e) {
    ...
    public void testCheckAndDeleteDiffRow() throws Exception {
        ...
        try (Table table = getDefaultTable()) {
            ...
            checkAndDelete(rowKey2, COLUMN_FAMILY, qual, null, delete);
        } catch (IOException e) {
            assertGetRowException(e);
        }
    }
    ...
    public void testCheckAndMutate() throws Exception {
        ...
        success =
            checkAndMutate(
                rowKey, COLUMN_FAMILY, qualCheck, CompareOp.EQUAL, null, rm);
        Assert.assertTrue(success);
        ...
        Assert.assertTrue(
            "QualPut should exist", row.containsColumn(COLUMN_FAMILY, qualPut));
        table.close();
    }
    ...
    public void testCompareOpsNull() throws Exception {
        ...
        success =
            checkAndPut(
                rowKey, COLUMN_FAMILY, nullQual, CompareOp.LESS, null, someRandomPut);
